from unittest.mock import patch
from src.file_transactions import get_transactions_file_csv, get_transactions_file_xlsx, PATH_CSV, PATH_XLSX


# Тест успешного чтения .cvs
def test_get_transactions_file_csv() -> None:
    with patch("pandas.read_csv") as mock_read:
        mock_read.return_value.to_dict.return_value = {
                                                        'id': 650703.0,
                                                       'state': 'EXECUTED',
                                                       'date': '2023-09-05T11:30:32Z',
                                                       'amount': 16210.0,
                                                       'currency_name': 'Sol',
                                                       'currency_code': 'PEN',
                                                       'from': 'Счет 58803664561298323391',
                                                       'to': 'Счет 39745660563456619397',
                                                       'description': 'Перевод организации'
                                                       }
        result = get_transactions_file_csv(PATH_CSV)
        assert result == {
                            'id': 650703.0,
                           'state': 'EXECUTED',
                           'date': '2023-09-05T11:30:32Z',
                           'amount': 16210.0,
                           'currency_name': 'Sol',
                           'currency_code': 'PEN',
                           'from': 'Счет 58803664561298323391',
                           'to': 'Счет 39745660563456619397',
                           'description': 'Перевод организации'
                           }


# Тест, когда файл .csv не найден
def test_get_transactions_file_csv_missing():
    with patch('builtins.open', side_effect=FileNotFoundError):
        result = get_transactions_file_csv("")
        assert result == []  # Проверим, что вернулся пустой список


# Тест, когда ошибка при чтении файла .csv
@patch('src.file_transactions.pd.read_csv', side_effect=Exception("Ошибка при чтении CSV:"))
def test_csv_error(mock_read_csv):
    """
    Проверяет реакцию функции на ошибку чтения CSV-файла.
    """
    result = get_transactions_file_csv(PATH_CSV)
    assert result == []  # 'Ожидался пустой список, получил {result}'


# Тест успешного чтения .xlsx
def test_get_transactions_file_xlsx() -> None:
    with patch("pandas.read_excel") as mock_read:
        mock_read.return_value.to_dict.return_value = {
                                                        'id': 650703.0,
                                                       'state': 'EXECUTED',
                                                       'date': '2023-09-05T11:30:32Z',
                                                       'amount': 16210.0,
                                                       'currency_name': 'Sol',
                                                       'currency_code': 'PEN',
                                                       'from': 'Счет 58803664561298323391',
                                                       'to': 'Счет 39745660563456619397',
                                                       'description': 'Перевод организации'
                                                       }
        result = get_transactions_file_xlsx(PATH_XLSX)
        assert result == {
                            'id': 650703.0,
                           'state': 'EXECUTED',
                           'date': '2023-09-05T11:30:32Z',
                           'amount': 16210.0,
                           'currency_name': 'Sol',
                           'currency_code': 'PEN',
                           'from': 'Счет 58803664561298323391',
                           'to': 'Счет 39745660563456619397',
                           'description': 'Перевод организации'
                           }

# Тест, когда файл .xlsx не найден
def test_get_transactions_file_xlsx_missing():
    with patch('pandas.read_excel', side_effect=FileNotFoundError):
        result = get_transactions_file_xlsx("no_file.xlsx")
        assert result == []  # Проверим, что вернулся пустой список

@patch('src.file_transactions.pd.read_excel', side_effect=Exception('Ошибка при чтении XLSX:'))
def test_xlsx_error(mock_read_xlsx):
    """
    Проверяет реакцию функции на ошибку чтения XLSX-файла.
    """
    result = get_transactions_file_xlsx(PATH_XLSX)
    assert result == []  # 'Ожидался пустой список, получил {result}'