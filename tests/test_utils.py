...
def test_get_amount_transactions_in_rub():
    with patch('requests.get') as r_mock:
        r_mock.return_value.json.return_value = {"result":123}
        assert get_amount_transactions_in_rub({"operationAmount": {"amount": "79114.93", "currency": {"code": "USD"}}}) == 123

    assert get_amount_transactions_in_rub({"operationAmount": {"amount": "34114.93", "currency": {"code": "RUB"}}}) == "34114.93"

    assert get_amount_transactions_in_rub({"operationAmount": {"amount": "34114.93", "currency": {"code": "GBR"}}}) == 'Неизвестная валюта'